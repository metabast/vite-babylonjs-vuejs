...
class MatcapTweaks {
    ...
    addTweaks() {
        ...
        this.folder.addInput(this.params, "matcapId", {
            label: "id"
        });

        this.folder.addInput(this.params, "matcapIndex", {
                label: "index",
                min: 0,
                max: MatcapEditor.matcapIds.length - 1,
                step: 1
            })
            .on("change", (event) => {
        ...
        this.folder.addInput(this.params, "matcapDesaturate", {
            label: "desaturate"
        }).on("change", (event) => {
            MatcapEditor.updateMatcapDesaturate(event.value);
        });
        this.folder.addInput(this.params, "matcapBump", {
            label: "bump",
            min: 0,
            max: 10,
            step: .1
        }).on("change", (event) => {
            window.dispatchEvent(new CustomEvent("matcap:bump", {
                detail: event.value
            }));
        });

        this.folder.addInput(this.params, "matcapBlur", {
            label: "matcapBlur",
            min: 0,
            max: 180,
            step: 1
        ...
        this.folder.addInput(this.params, "matcapBrightness", {
            label: "matcapBrightness",
            min: -100,
            max: 100,
            step: 1
        }).on("change", (event) => {
        ...
        this.folder.addInput(this.params, "matcapContrast", {
            label: "matcapContrast",
            min: -100,
            max: 100,
            step: 1
        }).on("change", (event) => {
        ...
        this.folder.addInput(this.params, "matcapEmissiveColor", {
            label: "emissive"
        }).on("change", (event) => {
            // BoardMesh.matcapMaterial.emissiveColor = Color3.FromHexString(event.value);
            window.dispatchEvent(new CustomEvent("matcap:emissiveColor", {
                detail: event.value
            }));
        });

        this.folder.addInput(this.params, "matcapReflectionColor", {
            label: "reflection"
        }).on("change", (event) => {
            // BoardMesh.matcapMaterial.reflectionColor = Color3.FromHexString(event.value);
            window.dispatchEvent(new CustomEvent("matcap:reflectionColor", {
                detail: event.value
            }));
        });
